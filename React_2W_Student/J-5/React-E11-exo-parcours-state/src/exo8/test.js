 ...
 * Export default
 */
export default (createStore, reducer, changeName) => {
  const store = createStore(reducer);
  if (!(store && store.getState && store.dispatch && changeName)) {
    test(false);
    return;
  }
  ...
  store2.dispatch(changeName('Bureau'));
  store2.dispatch({ type: 'SWITCH_OFF' });
  ...
  test((
    ...
    && _.isEqual(changeName('hello'), { type: 'CHANGE_NAME', name: 'hello' })
